...
namespace needle
{
  namespace cpu
  {
// TODO: Correct tiles & Alignment
#define ALIGNMENT 256
#define TILE 8
#define TILE_SIZE TILE *TILE
                           ...
                           float *__restrict__ out)
    {
      ...
      for (size_t i = 0; i < TILE; i++)
      {
        ...
        {
          // Load 8 floats
          // __m256 sum = _mm256_load_ps(out + i * TILE + j);
          // TODO: Something is wrong with the iteration - first element is correct,
          // others not
          for (size_t k = 0; k < TILE; k++)
          {
            out[i * TILE + j] += a[i * TILE + k] * b[k * TILE + j];
            // // Load 8 floats
            // __m256 vec_a = _mm256_load_ps(a + i * TILE + k);
            // __m256 vec_b = _mm256_load_ps(b + k * TILE + j);

            // // FMA (a * b + c) instruction
            // sum = _mm256_fmadd_ps(vec_a, vec_b, sum);
          }
          // _mm256_store_ps(out + i * TILE + j, sum);
        }
      }
    }
                     ...
                     AlignedArray *out, uint32_t m, uint32_t n, uint32_t p)
    {
      ...
      for (size_t i = 0; i < a_rows; i++)
      {
        ...
        {
          // block[i][j][:][:]
          float *out_block = (scalar_t *)(out->ptr + TILE_SIZE * (i * b_cols + j));
          for (size_t k = 0; k < a_cols; k++)
          {
            // tile multiplication: a[i][j] * b[i][j]
            float *tile_a = (scalar_t *)(a.ptr + TILE_SIZE * (i * a_cols + k));
            float *tile_b = (scalar_t *)(b.ptr + TILE_SIZE * (k * b_cols + j));
