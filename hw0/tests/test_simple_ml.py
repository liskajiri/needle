...
def test_softmax_regression_epoch_rust():
    # test numerical gradient
    from hw0 import softmax_regression_epoch_rust
    ...
    Theta = np.zeros((5, 3), dtype=np.float32)
    dTheta = -nd.Gradient(lambda Th: softmax_loss(X @ Th.reshape(5, 3), y))(Theta)
    softmax_regression_epoch_rust(X, y, Theta, 50, 5, 3, lr=1.0, batch=50)
    np.testing.assert_allclose(dTheta.reshape(5, 3), Theta, rtol=1e-4, atol=1e-4)
    ...
    theta = np.zeros((X.shape[1], y.max() + 1), dtype=np.float32)
    softmax_regression_epoch_rust(X[:100], y[:100], theta, lr=0.1, batch=10)
    np.testing.assert_allclose(np.linalg.norm(theta), 1.0947356, rtol=1e-5, atol=1e-5)
