       ...
       */
        ...
        for (size_t i = 0; i < a_rows; i++) {
            for (size_t j = 0; j < b_cols; j++) {
                // block[i][j][:][:]
                auto out_block = (scalar_t *) (out->ptr + TILE_SIZE * (i * b_cols + j));
                for (size_t k = 0; k < a_cols; k++) {
                    // tile multiplication: a[i][j] * b[i][j]
                    auto tile_a = (scalar_t *) (a.ptr + TILE_SIZE * (i * a_cols + k));
                    auto tile_b = (scalar_t *) (b.ptr + TILE_SIZE * (k * b_cols + j));
