...
_ALL_DEVICES = [nd.cpu(), pytest.param(nd.cuda(), 
    marks=pytest.mark.skipif(not nd.cuda().enabled(), reason="No GPU")), nd.cpu_numpy()]
...
@pytest.mark.parametrize("m,n,p", matmul_dims)
def test_matmul_benchmark(benchmark, m, n, p, device):
    def pseudo_matmul(A, B):
        return A @ B
    ...
    out = benchmark(pseudo_matmul, A, B)
